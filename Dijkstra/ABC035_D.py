...
def main():
    ...
    for _ in range(M):
        ...
        reversed_graph[b - 1].append([c, a - 1])
    dijkstra = Dijkstra(N, M, Graph)
    reversed_dijkstra = Dijkstra(N, M, reversed_graph)
    going_dist, _ = dijkstra.GetShortestPaths(0)
    returning_dist, _ = reversed_dijkstra.GetShortestPaths(0)
    ans = 0
    for i in range(N):
        point = A[i] * (T - going_dist[i] - returning_dist[i])
        ans = max(ans, point)
    print(ans)
